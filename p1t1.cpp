...
double sinTaylorH(const int N, double x) {
  vector<double> a (2*N); //numb. of polyn. coeficients should be the deg. + 1
  for (int n = 0; n < N; n++)
...
// taylor expansion for sin with Horner's scheme
double cosTaylorH(const int N, double x) {
  vector<double> b (2*N); //numb. of polyn. coeficients should be the deg. + 1
  for (int n = 0; n < N; n++)
    b[2*n] = ( (1-2*(n%2)) / factorial(2 * n ));
  return hornerScheme(b,2*N-2, x);
}
